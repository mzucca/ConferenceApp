...
namespace ReHub.Data.Tests
{
    public class Tests
    {
        static readonly string myActualKey = "rehub_encrypt";
        static readonly string myPassword = "123456789";
        [SetUp]
        public void Setup()
        {
        }
        ...
        public void EncryptPassword()
        {
            ...
            var provider = new GenerateEncryptionProvider(myActualKey,EncryptionAlgorithm.Aes);
            var encrypted = provider.Encrypt(myPassword);

            var decrypted = provider.Decrypt(encrypted);
            Assert.That(myPassword,Is.EqualTo(decrypted));
        }
        [Test]
