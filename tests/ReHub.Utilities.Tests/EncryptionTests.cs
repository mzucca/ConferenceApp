...
namespace ReHub.Utilities.Tests
{
    ...
    public class EncryptionTests
    {
        ...
        [TestMethod]
        public void TestEncryptionSameSalt()
        {
            // Encryption
            var salt = new byte[] { 246, 38, 38, 179, 164, 194, 198, 226 };
            Encryption.Encryption encrypt1 = new Encryption.Encryption("Password123",salt, EncryptionAlgorithm.Aes);
            var enc1 = encrypt1.Encrypt(text);

            Encryption.Encryption encrypt2 = new Encryption.Encryption("Password123",salt, EncryptionAlgorithm.Aes);
            var enc2 = encrypt2.Encrypt(text);

            Assert.AreEqual(enc1, enc2);

            var dec = encrypt2.DecryptString(enc1);
            Assert.AreEqual(text, dec);

        }
        [TestMethod]
        public void TestEncryptionOverDifferentInstances()
        {
            Encryption.Encryption encrypt1 = new Encryption.Encryption("Password123", EncryptionAlgorithm.Aes);
            var enc1 = encrypt1.Encrypt(text);

            Encryption.Encryption encrypt2 = new Encryption.Encryption("Password123", EncryptionAlgorithm.Aes);
            var enc2 = encrypt2.DecryptString(enc1);

            Assert.AreEqual(enc2, text);
        ...
        public void TestEncryption()
        {
            ...
            {
                Encryption.Encryption encrypt = new Encryption.Encryption("Password123", algorithm);
