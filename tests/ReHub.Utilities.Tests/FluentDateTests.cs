...
namespace ReHub.Utilities.Tests
{
    ...
    {
        static Dictionary<int, Func<int, int, FluentDate>> fluentDatesFull = new()
        {
        ...
        static Dictionary<int, Func<int, int, FluentDate>> fluentDatesShort = new()
        {
        ...
        public void Dates(int month, Func<int, int, FluentDate> getFluentDate)
        {
        ...
        [Theory]
        [MemberData(nameof(FluentDates))]
        public void DateAt(int month, Func<int, int, FluentDate> getFluentDate)
        {
            var date = GetRandomDateTimeOfMonth(month).DateTime;
            var fluentDate = getFluentDate(date.Year, date.Day);
        ...
        [Theory]
        [MemberData(nameof(FluentDates))]
        public void DateAtWithKind(int month, Func<int, int, FluentDate> getFluentDate)
        {
            var date = GetRandomDateTimeOfMonth(month).DateTime;
            var fluentDate = getFluentDate(date.Year, date.Day);
        ...
        [Theory]
        [MemberData(nameof(FluentDates))]
        public void DateAtTimeZone(int month, Func<int, int, FluentDate> getFluentDate)
        {
            var date = GetRandomDateTimeOfMonth(month);
            var fluentDate = getFluentDate(date.Year, date.Day);

            Xunit.Assert.Equal(date, fluentDate.At(date.Offset, date.Hour, date.Minute, date.Second, date.Millisecond));
        }
